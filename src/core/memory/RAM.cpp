...
RAM::RAM(DS* ds)
{
	...
	this->memoryMap = new Memory[this->memoryMapSize = 25]();
...
bool RAM::load(uint8_t* data, uint32_t dataLength, uint32_t destAddress)
{
	...
	if(destAddress + dataLength > memory->getMemorySize())
	{
		Logger::log("Attempted to load into RAM at address: " + String::decToHex(memory->getStartAddress() + destAddress) + " to " + String::decToHex((memory->getStartAddress() + destAddress) + dataLength) + " (Max Memory: " + String::decToHex(memory->getMemorySize() /* TODO: Calculate total RAM */) + ")");
...
bool RAM::copy(uint32_t sourceAddress, uint32_t destAddress, uint32_t length)
{
	...
	if(destAddress + length > destMemory->getMemorySize())
	{
...
{
	for(uint32_t index = 0; index < this->memoryMapSize; index++)
	{
...
uint32_t RAM::getTotalRAMSize()
{
	...
	for(uint32_t index = 0; index < this->memoryMapSize; index++)
	{
		...
		totalRamSize += memory.getMemorySize();
	}
...
uint8_t* RAM::getRAM()
{
	uint8_t* ram = new uint8_t[this->getTotalRAMSize()]();
	...
	for(uint32_t index = 0; index < this->memoryMapSize; index++)
	{
		...
		memcpy(ram + currentAddress, memory.getMemory(), memory.getMemorySize());
...
void RAM::print()
{
	...
	for(uint32_t index = 0; index < this->memoryMapSize; index++)
	{
		...
		string hex = String::toHexString(memory.getMemory(), memory.getMemorySize());
